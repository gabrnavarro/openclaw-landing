  ...
  return (
    <div className="min-h-screen bg-white text-neutral-950 dark:bg-neutral-950 dark:text-neutral-50">
      ...
      <main className="mx-auto w-full max-w-3xl px-6 py-16 sm:py-20">
        ...
        <article className="mt-10 overflow-hidden rounded-3xl border border-neutral-200 bg-white shadow-sm dark:border-neutral-700 dark:bg-neutral-800">
          <header className="border-b border-neutral-100 p-6 sm:p-8 dark:border-neutral-700">
          ...
          <div
            className="prose prose-neutral blog-prose max-w-none p-6 sm:p-8 prose-headings:tracking-tight prose-a:text-blue-600 prose-a:no-underline hover:prose-a:underline dark:prose-a:text-blue-400"
        ...
        <nav className="mt-8 flex items-center justify-between gap-4 text-sm">
          <div>
            {newer ? (
              <Link className="text-neutral-700 hover:underline dark:text-neutral-300" href={`/blog/${newer.slug}`}>
                ← {newer.title}
              </Link>
            ) : null}
          </div>
          <div className="text-right">
            {older ? (
              <Link className="text-neutral-700 hover:underline dark:text-neutral-300" href={`/blog/${older.slug}`}>
                {older.title} →
              </Link>
            ) : null}
          </div>
